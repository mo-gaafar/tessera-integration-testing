...
public class loginEmptyPass {
    public static void main(String[] args) throws MalformedURLException, InterruptedException {
        ...
        // click the login button with empty password
        WebElement loginBtn = driver.findElement(By.xpath("//android.widget.Button[@content-desc=\"Log in\"]"));
        loginBtn.click();

        // empty password alert
        Thread.sleep(1000);
        WebElement passwordAlert = driver.findElement(By.xpath("//android.view.View[@content-desc=\"Email and password are required\"]"));
        String passwordAlertText = passwordAlert.getAttribute("content-desc");
        if(passwordAlertText.equals("Email and password are required"))
        {
            System.out.println("Empty Password Alert Passed");
        }
        else
        {
            System.out.println("Empty Password Alert Failed");
        }
