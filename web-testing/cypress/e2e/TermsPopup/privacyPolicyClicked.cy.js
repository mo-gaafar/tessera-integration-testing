...
describe("Signup Page 2 - Normal case", () => {
  it.only("Validate terms and conditions popup elements exist", () => {
    ...
    cy.get(termObj.guidelinesLink).should(
      "have.attr",
      "rel",
      "noopener noreferrer"
    );
    cy.get(termObj.privacyPolicyLink).invoke("removeAttr", "target");
    cy.get(termObj.privacyPolicyLink).click();
    cy.get(termObj.tosPageTitle).should("exist");
    cy.get(termObj.tosPageTitle).should("have.text", "Eventbrite Privacy Policy");
